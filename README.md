...
A versão 0.1 implementa a base Windows, impressão RAW/ESC-POS, CP850, bandeja, autostart, configurações, logs, pareamento seguro, renovação automática da sessão e worker da fila. A partir da versão 0.1.8, o cabeçalho publicitário monocromático da ZYRON é gerado em raster dentro do aplicativo, funciona offline e respeita a opção configurada pela loja. A versão 0.1.10 adota a nova marca horizontal e também imprime o logotipo na comanda de teste. A versão 0.1.11 elimina a margem vazia inferior da marca e força cada adicional do item a ocupar sua própria linha. A versão 0.1.12 remove da comanda a indicação redundante “PAGAR NA ENTREGA”. O Delivery mantém o QZ Tray como modo de compatibilidade e permite selecionar o ZYRON Print por loja.
